...
'''
    ...
    for num_skills in range(4, 12):
        ...
        for i in range(10):
            ...
            total_max += len(max_logit_team)
            # Tfs
            tfs_team = str(tfs_team)
            print('Tfs team', tfs_team)
            pairs = tfs_team.split(":")
            total_tfs += len(pairs)

            print('cbr team', cbr_team)
            total_cbr += len(cbr_team)
        ...
        average_team_max_logit = total_max / 10
        average_team_tfs = total_tfs  / 10
        average_team_cbr = total_cbr / 10
        y_values_max_logit.append(average_team_max_logit )
        y_values_tfs.append(average_team_tfs)
        y_values_cbr.append(average_team_cbr)
